...
public class VillageDataHandler
{
    ...
    {
        if (!VillageInfo.SHOW_VILLAGE || villageDataList == null || villageDataList.size() == 0)
            return;
        ...
        for (VillageData data : villageDataList)
        {
            ...
            GlStateManager.pushMatrix();
            {
                ...
                {
                    if (VillageInfo.VILLAGE_SPHERE)
                        drawBorderSpehere(data.radius, 16, 64, buffer, VillageInfo.PER_VILLAGE_COLOR ? COLORS[cnt % COLORS.length] :
                                new Vector4f(1f, 0f, 1f, 1f));
                    if (VillageInfo.VILLAGE_DOORS)
                        drawDoors(data.center, data.doorPositions, buffer, VillageInfo.PER_VILLAGE_COLOR ? COLORS[cnt % COLORS
                                .length] : new Vector4f(1f, 1f, 1f, 1f));
                    if (VillageInfo.VILLAGE_BORDER)
                        drawBorderSquare(data.radius, buffer, VillageInfo.PER_VILLAGE_COLOR ? COLORS[cnt % COLORS.length] : new
                                Vector4f(1f, 1f, 0f, 0.5f));
                    if (VillageInfo.VILLAGE_GOLEM)
                        drawGolemSpawn(buffer, VillageInfo.PER_VILLAGE_COLOR ? COLORS[cnt % COLORS.length] : (canSpawn ? new Vector4f
                                (0f, 1f, 0f, 1f) : new Vector4f(0.8f, 0f, 0f, 1f)));
                    if (VillageInfo.VILLAGE_CENTER)
                        drawCenter(buffer, VillageInfo.PER_VILLAGE_COLOR ? COLORS[cnt % COLORS.length] : new Vector4f(1f, 0f, 0f,
                                1f));
                    if (VillageInfo.VILLAGE_INFO_TEXT)
                    {
